...
public class RemoveDuplicatesShuffle {
  ...
  public static int compress(int[] array) {
    ...
    if (arraySize > 0) {
      int[] a = array;
      IntVector _prev = IntVector.broadcast(SP, a[0]);

      for (int i = 1; i < arraySize; i += vl) {
        var _mask = SP.indexInRange(i, arraySize);
        var _current = IntVector.fromArray(SP, a, i, _mask);
        var _shifted = _prev.slice(vl - 1, _current);
        _prev = _current;
        ...
        _current.compress(_neq).intoArray(a, writeIndex, _write);
