...
public class LargestProductInSeriesPreprocessed {
  ...
  public static long compute() {
    var data = thousandDigits.toCharArray();
    ...
    for (int i = 0; i < data.length; i += vl) {
      var chars = ShortVector.fromCharArray(
        CHAR_SPECIES, data, i, _mask);
      var longs = chars.convertShape(CONV, SP, 0)
